...
async def main():
    ...
    log(f"전체: {len(all_events)}경기 / 기수집: {len(done)} / 0-0스킵: {len(zero_zero)} / 수집대상: {len(targets)}경기")
            ...
            for inc in data["incidents"]:
                ...
                try:
                    cur.execute("""
                        ...
                        (event_id, team_id, player_id, player_name, minute, added_time, is_home, is_own_goal, is_penalty)
                        VALUES (?,?,?,?,?,?,?,?,?)
                    """, (eid, team_id, pid, pname, minute, added_time, is_home, is_own, is_pen))
