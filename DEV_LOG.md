...
### K리그1 선수 한국어 이름 최신화 `00:30`

**변경 파일**
- `crawlers/fill_k1_player_names.py` — K1 출전 선수 225명 players 테이블 신규 등록
- `crawlers/fill_ko_names_from_roster.py` — 등번호 매핑으로 한국어 이름 업데이트

**작업 내용**
1. Sofascore API → K1 2026 출전 선수 225명 신규 등록 (name_ko 없음 — API가 한국어 번역 미제공)
2. `data/kleague_players_2026.json` (portal.kleague.com 수집 데이터) × Sofascore 로스터 API 등번호 교차 매핑
3. K1 12개 팀 218명 name_ko 업데이트 완료
4. 여전히 미매핑 276명 (K1+K2 합산) — 외국인 선수·비등록 선수 포함
